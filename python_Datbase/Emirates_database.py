...
def data_trend():
    ...
    print("Top trend is in United Arab Emirates: ", data)

    for tweet in tweepy.Cursor(api.search, q=data + ' -filter:retweets', wait_on_rate_limit=True, lang='ar',
                               tweet_mode='extended').items():

        print(i, end='\r')
        p.set_options(p.OPT.URL, p.OPT.EMOJI)
        # print(p.clean(tweet.full_text))
        '''blob = TextBlob(p.clean(tweet.full_text))
        if blob.sentiment.polarity > 0:
            ...
            print("سلبي ")
            count_neqative = count_neqative + 1
            blob = "سلبي "
        else:
            print("طبيعي ")
            counnt_nurmal = counnt_nurmal + 1
            blob = "طبيعي "
'''
