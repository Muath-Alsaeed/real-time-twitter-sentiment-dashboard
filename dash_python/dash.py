...
conn = sqlite3.connect('arab_database3.db', check_same_thread=False)
...
sentiment_colors = {-1: "#EE6055",
                   ...
                   0.5: "#D0F2DF",
                   1: "#9CEC5B", }
app_colors = { 'background': '#0C0F0A', 'text': '#FFFFFF',  'sentiment-plot': '#41EAD4', 'volume-bar': '#FBFC74',
   'someothercolor': '#FF206E', }
POS_NEG_NEUT = 0.1
MAX_DF_LENGTH = 100
app = dash.Dash(__name__)
app.layout = html.Div( [html.Div(className='container-fluid', children=[html.H2('Live Twitter Sentiment', style={'color': "#CECECE"}),
                                                    html.H5('Search:', style={'color': app_colors['text']}),
                                                    dcc.Dropdown( id='sentiment_term',  options=[
                                                            {'label': 'Saudi Arabia', 'value': 'sudi Arabi'},
                                                            {'label': 'United Arab Emirates',
                                                             'value': 'United Arab Emirates'},
                                                            {'label': 'Kuwait', 'value': 'Kuwait'},
                                                            {'label': 'Qatr', 'value': 'Qatr'},
   ...
   html.Div(className='row', children=[html.Div(dcc.Graph(id='live-graph', animate=False), className='col s12 m6 l6'),
    ...
    dcc.Interval( id='historical-update', interval=60 * 1000 ),
    dcc.Interval( id='related-update',  interval=30 * 1000),
   ...
   try:
    ...
    # it's way faster to join strings to one string then use regex split using your punctuation list plus space and new line chars
       ...
       for term, count in counts:
           try:
               df = pd.read_sql( "SELECT sentiment.* FROM  sentiment_fts fts LEFT JOIN sentiment ON fts.rowid = sentiment.id WHERE fts.sentiment_fts MATCH ? ORDER BY fts.rowid DESC LIMIT 200",
                   conn, params=(term,))
...
#this function to call the tweet from DAtabase and update reel time if the database still work 
def generate_table(df, max_rows=10):
   return html.Table(className="responsive-table",
                     children=[.Thead(html.Tr(children=[html.Th(col.title()) for col in df.columns.values],
                                 style={'color': app_colors['text']} ) ),
                         html.Tbody( [ html.Tr(  children=[html.Td(data) for data in d ], style={'color': app_colors['text'],
...
def update_recent_tweets(sentiment_term):
   ...
   df = df[['date', 'tweet', 'sentiment','country']]
...
#this function is used for cont how many tweet is Positive, Negative,natural
...
#this function is used for update the Volume  and sentiment in reet time and representation
is line 
def update_graph_scatter(sentiment_term):
   try:
      df = pd.read_sql("SELECT * FROM sentiment WHERE country LIKE ? ORDER BY unix DESC LIMIT 1000", conn,
       ...
       Y2 = df.volume.values
       data = plotly.graph_objs.Scatter(x=X, y=Y, name='Sentiment', mode='lines', yaxis='y2',
           line=dict(color=(app_colors['sentiment-plot']), width=4, ))
       data2 = plotly.graph_objs.Bar( x=X, y=Y2, name='Volume', marker=dict(color=app_colors['volume-bar']),)
       return {'data': [data, data2], 'layout':
                go.Layout(xaxis=dict(range=[min(X), max(X)]),
                                                          ...
                                                          font={'color': app_colors['sentiment-plot']},
                                                          plot_bgcolor=app_colors['background'],
                                                          paper_bgcolor=app_colors['background'],
                                                          showlegend=False)}
   except Exception as e:
   ...
   try:
       ...
       buttons = [html.H5('Recently Trending Terms: ', style={'color': app_colors['text']})] + [
           html.Span(term, style={'color': sentiment_colors[round(related_terms[term][0] * 2) / 2],
                                  'margin-right': '15px',
                                  'margin-top': '15px',
                                  'font-size': '{}%'.format(generate_size(related_terms[term][1], smin, smax))}) for
